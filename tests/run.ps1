# Builds after-effects-theme.wh.cpp with the compiler Windhawk ships, the way
...
$out = Join-Path ([IO.Path]::GetTempPath()) 'after-effects-theme-tests'
...
try {
    & $clang -std=c++23 -O2 -shared @defines '-DWH_MOD_ID=L"after-effects-theme"' `
        '-DWH_MOD_VERSION=L"1.0.0"' $lib.FullName `
        -x c++ (Join-Path $repo 'after-effects-theme.wh.cpp') -include windhawk_api.h `
        -target x86_64-w64-mingw32 '-Wl,--export-all-symbols' `
        -o (Join-Path $out 'after-effects-theme.dll') -ldwmapi -lgdi32 -Wall -Wextra
