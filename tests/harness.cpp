// Logic tests for after-effects-theme.wh.cpp, built with the Windhawk API
...
#include "../after-effects-theme.wh.cpp"
...
static void TestColorTable() {
    ...
    // "After Effects interface" off at runtime, "Direct fills" still on.
...
/*
    Reading four floats from something that is not a color would fault inside
    After Effects. Anything in the first page or misaligned for a float is
...
/*
    ...
    the interface, so answering an anchor cannot be enough on its own.
*/
...
static std::string ModSource() {
    ...
    path = path.substr(0, path.find_last_of("/\\") + 1) + "../after-effects-theme.wh.cpp";
...
static void TestPaletteRules() {
    for (const NamedPalette& named : kPalettes) {
        ...
        /*
            A highlight is a hue, not a tone: the shade each of the interface
...
/*
    ...
    for, not the name on its own: "After Effects" is the name of the
    application on almost every line.
*/
static void TestPaletteReadme() {
    ...
    for (const NamedPalette& named : kPalettes) {
        ...
        // The one palette whose name is two words.
        if (id == L"aftereffects") {
            column = "| After Effects";
        }
...
static void TestShippedDefaults() {
    // The UXP layer ships off: its effect only a restart undoes, so it has
...
static void TestCustomTheme() {
    ...
    for (const wchar_t* key : {L"base", L"panel", L"surface", L"raised", L"border",
                               L"text", L"accent", L"disabledText",
                               L"highlight"}) {
        bool found = false;
...
static void TestHighlight() {
    ...
    // The Timeline's frame count, which After Effects derives rather than
    // takes from a table, is one of them.
    const DvaColorRGBA frames = GdiToDva(RGB(0x0B, 0x59, 0xAD));
    CHECK(InterfaceBlueIndex(frames) >= 0);

    // The palettes without a highlight leave the interface blue alone.
    g_fakePalette = L"onyx";
    LoadSettings();
    CHECK(!CurrentSettings().highlight);
    ...
    for (const auto& t : kTrack) {
        ...
        CHECK(ConvertDvaColor(frames, &out));
...
static void TestBundledStylesheetPath() {
    SetUxpPluginsDirFrom(
        L"C:\\Program Files\\Adobe\\Adobe After Effects 2026\\Support Files"
        L"\\AfterFX.exe");

    LPCWSTR relative = nullptr;
    CHECK(BundledFile::Stylesheet == BundledFileKind(L"C:\\Program Files\\Adobe\\Adobe After Effects 2026\\Support Files"
                              L"\\UXP\\plugins\\com.adobe.ccx.start\\css\\app.css",
                              &relative));
    CHECK(relative && wcscmp(relative, L"com.adobe.ccx.start\\css\\app.css") == 0);

    CHECK(BundledFile::Stylesheet == BundledFileKind(
        L"c:/program files/adobe/adobe after effects 2026/support files/uxp/plugins/p/main.CSS", nullptr));
    CHECK(BundledFile::Stylesheet == BundledFileKind(L"\\\\?\\C:\\Program Files\\Adobe\\Adobe After Effects "
                              L"2026\\Support Files\\UXP\\plugins\\p\\a.css",
                              nullptr));

    // The panel's script is bundled too, and carries the design tokens.
    CHECK(BundledFileKind(L"C:\\Program Files\\Adobe\\Adobe After Effects 2026\\Support Files"
    ...
    // So is a webview's page, which the panel serves from the same folder.
    CHECK(BundledFileKind(L"C:\\Program Files\\Adobe\\Adobe After Effects 2026\\Support Files"
                          L"\\UXP\\plugins\\p\\uab\\webview.html",
                          nullptr) == BundledFile::Page);
    CHECK(BundledFileKind(L"C:\\Program Files\\Adobe\\Adobe After Effects 2026\\Support Files"
                          L"\\UXP\\plugins\\p\\a.htm",
                          nullptr) == BundledFile::None);
    CHECK(BundledFile::None == BundledFileKind(L"C:\\Program Files\\Adobe\\Adobe After Effects 2026\\Support Files"
...
/*
    The whole of a script's rewrite, as After Effects 26.3's Home screen needs
    it: the stylesheet text a component injects, and a theme's table of
    backgrounds, taken like a stylesheet's — while an icon's own hex stays an
    icon, and a token is converted once, not twice.
*/
static void TestScriptCss() {
    g_fakePalette = L"onyx";
    SetFakeInt(L"ceiling", 28);
    SetFakeInt(L"strength", 100);
    LoadSettings();

    auto rgb = [](int level, size_t width) {
        return Triplet(Convert8(RGB(level, level, level)), width);
    };

    char js[] =
        "push([n.i,\".spectrum--dark  {\\n  background-color: rgb(50, 50, 50);\\n}\"]);"
        "const h={[p.THEME_DARKEST]:\"rgb(30, 30, 30)\",[p.THEME_LIGHT]:\"rgb(245, 245, "
        "245)\"};"
    ...
    // Injected stylesheet text, a theme's table, and a custom property in it.
    CHECK(s.find("background-color: rgb(" + rgb(50, 10) + ")") != std::string::npos);
    CHECK(s.find("THEME_DARKEST]:\"rgb(" + rgb(30, 10) + ")\"") != std::string::npos);
    CHECK(s.find("--host-background-color-darkest: " +
                 Hex(Convert8(RGB(0x32, 0x32, 0x32)), false)) != std::string::npos);
    CHECK(s.find("\"rgb(245, 245, 245)\"") != std::string::npos);  // light stays light

    // A token, once: a second conversion would push it past its own tone.
    CHECK(s.find("\"background-color\":\"rgb(" + rgb(37, 10) + ")\"") !=
          std::string::npos);

    // An icon's hex is left alone; a quoted hex under a chrome key is not.
    CHECK(s.find("fill:\"#2c2c2c\",stroke:\"#231f20\"") != std::string::npos);
    ...
    /*
        A key in quotes is the property it names, in a stylesheet too: UXP
        ships its rules serialized, and a border there gets the chrome slack.
    */
    char cssom[] = "{\"style\":{\"border-color\":\"#494949\",\"color\":\"#494949\"}}";
    const std::string beforeCssom = cssom;

    CHECK(RecolorStylesheet(cssom, beforeCssom.size()) == 1);
    CHECK(std::string(cssom).find("\"border-color\":\"" +
                                  Hex(ConvertChrome8(RGB(0x49, 0x49, 0x49)), false)) !=
          std::string::npos);
    CHECK(std::string(cssom).find("\"color\":\"#494949\"") != std::string::npos);
}
...
static void TestStylesheetRedirect() {
    ...
    SetUxpPluginsDirFrom((root + L"\\AfterFX.exe").c_str());
...
/*
    A value the mod produced is also a real interface gray: Onyx turns #3F3F3F
...
/*
    ...
    are, and After Effects makes them constantly — has no caption, so setting
